...
The response contains a single keyed morph for `"count"` instead of the full page. The client finds the element with `id="count"` (a Dynamic key renders as the element's id) and morphs only that subtree.
