...
When the client reconnects to the same node, the session is reattached: the transport is swapped, any stored snapshots are re-imported and the store entry deleted, and one diff against the preserved baseline sends exactly what changed while the client was away. The browser's URL and title are replayed too (they live outside the DOM and would otherwise desync). Side effects raised during the window - `Toast`, `Signal`, `Flash` - have no equivalent catch-up: they are dropped with a `CommandDiscarded` diagnostic.
