...
// Store persists differ snapshots for disconnected sessions. The
// framework calls Save when a session disconnects and Delete when it
// reconnects or is destroyed, allowing snapshot data to live outside
// process memory during the reconnect window.
//
...
//
// The framework does not ship any Store implementations. Developers
// provide their own, backed by whatever storage suits their deployment
// (SQLite, Redis, filesystem, etc.). The default (nil on [Config])
// keeps everything in memory as it does today.
//
// The data passed to Save is an opaque blob produced by the differ's
// Export method. Store implementations must not interpret or modify the
