...
// by the "poly" struct tag; untagged exported fields use their
...
func (e Event) Bind(dest any) error {
	...
	if rv.Kind() != reflect.Pointer || rv.IsNil() {
		return fmt.Errorf("poly: Bind requires a non-nil pointer, got %T", dest)
	}
	rv = rv.Elem()
	if rv.Kind() != reflect.Struct {
		return fmt.Errorf("poly: Bind requires a pointer to a struct, got *%s", rv.Type())
	}
	rt := rv.Type()
...
func setField(fv reflect.Value, raw, key string) error {
	...
	case reflect.Int, reflect.Int64:
		...
		if err != nil {
			return fmt.Errorf("poly: field %q: %w", key, err)
		}
		fv.SetInt(n)
	...
	case reflect.Bool:
		fv.SetBool(raw == "true")
	default:
		return fmt.Errorf("poly: unsupported field type %s for key %q", fv.Type(), key)
	}
	return nil
}
