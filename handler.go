...
type Config[S any] struct {
	...
	// Fallback converts an HTTP request into a Transport connection when
	// the primary WebSocket transport is unavailable. When set, the
	// handler also accepts SSE connections (Accept: text/event-stream)
	// and routes POST requests to active sessions via the EventPusher
	// interface. Use sse.Upgrade() for SSE+POST connections.
...
// ServeHTTP routes requests by type: WebSocket upgrades and SSE streams
// start a session event loop, POST requests push events to active
// sessions, and plain GET requests serve the initial HTML page.
